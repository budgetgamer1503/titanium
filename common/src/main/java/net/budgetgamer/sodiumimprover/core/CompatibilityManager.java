...
public final class CompatibilityManager {
    ...
    private static final Logger LOGGER = Logger.getLogger("SodiumImprover-Compat");
    ...
    public static synchronized void init() {
        ...
        LOGGER.info("[Sodium Improver] Compatibility layer initialized.");
        LOGGER.info("[Sodium Improver] Sodium detected: " + sodiumLoaded);

        if (entityCullingLoaded) {
            LOGGER.info("[Sodium Improver] Synergy active: EntityCulling detected. Sodium Improver handles fast frustum & dynamic distance culling to minimize background raytrace overhead.");
        }
        if (moreCullingLoaded) {
            LOGGER.info("[Sodium Improver] Synergy active: MoreCulling detected. Coordinated block entity and sign culling enabled.");
        }
        if (ebeLoaded) {
            LOGGER.info("[Sodium Improver] Synergy active: Enhanced Block Entities detected. Chunk-baked block entities will bypass redundant dispatcher checks.");
        }
        if (immediatelyFastLoaded) {
            LOGGER.info("[Sodium Improver] Synergy active: ImmediatelyFast detected. Immediate mode rendering optimized alongside Sodium Improver.");
        }
        if (rsoLoaded) {
            LOGGER.info("[Sodium Improver] Reese's Sodium Options detected. Dedicated vertical tab injected.");
        }
    }
